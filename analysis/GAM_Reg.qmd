...
# Generalised Additive Model Regression (Bayesian & Frequentist)
...
# hgd() # Opens port for httpgd visualisations
```
# Simulate Data
...
```{r}
ggplot(Raw2, aes(Time, Expl, group = n, color = Pheno))+
  geom_line(linewidth =1)+ylim(0,100) +
  scale_color_manual(values = c("#9c1f1f", "#8389a8", "#0e3063"))+ 
  xlab("Time (s)") + ylab("Maze Explored (%)")+
  theme_classic()
...
```{r}
# Make Pheno and n factors for gam model
# make factors
Raw <- Raw |>
  dplyr::mutate(
    Pheno = factor(Pheno),
...
draw(fit.gam.freq, residuals = TRUE)
